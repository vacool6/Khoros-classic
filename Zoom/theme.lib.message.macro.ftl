...
<#-- Start: Code added by ajay to verify wether user has Kudoed it or not  -->
    <#assign is_kudoed = "false" />
    <#assign verifyKudo= rest("2.0","/search?q="+"Select id from kudos where user.id = '${user.id}' and message.id='${msg.id}'"?url)/> 
    <#if verifyKudo.data.size?number gt 0>
        <#assign is_kudoed = "true" />
    </#if>
    <#--  Ajay code Ends here  -->
    <ul class="custom-tile-statistics">
         <#--  Sammer mods  -->
        <li class="custom-tile-views"><b>${msg.metrics.views}</b> ${text.format("general.Views")}</li>
        <li user-id="${user.id}" view-href="${msg.view_href}" class="custom-tile-replies"><b>${msg.replies.count}</b> ${text.format("general.replies")}</li>
        <#if (is_kudoed == "true")>
            <li msg-id="${msg.id}" class="custom-tile-kudos-liked ${is_kudoed}"><b>${msg.kudos.sum.weight}</b> ${text.format("general.kudos")}</li>
         <#else>
            <li msg-id="${msg.id}" class="custom-tile-kudos ${is_kudoed}"><b>${msg.kudos.sum.weight}</b> ${text.format("general.kudos")}</li>
        </#if>
         <#--  Sammer mods  -->
    </ul>
</#macro>
